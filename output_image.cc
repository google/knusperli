...
void OutputImageComponent::CopyFromJpegComponent(const JPEGComponent& comp,
  ...
  const size_t coeffs_size =
      width_in_blocks_ * height_in_blocks_ * kDCTBlockSize;

  // For all coefficients in all blocks, compute their allowed range and the
  // naive mid value that a standard decoder would use. We will later tweak the
  // mid values, while keeping them within the min/max range. The modifications
  // to the mid offset are stored separately (in blocks_off), to ensure that
  // modification to one block does not yet affect evaluation of the next block;
  // modifications to each block are accumulated before they are applied.
  coeff_t* blocks_min = new coeff_t[coeffs_size];
  coeff_t* blocks_max = new coeff_t[coeffs_size];
  coeff_t* blocks_mid = new coeff_t[coeffs_size];
  int64_t* blocks_off = new int64_t[coeffs_size];

  for (int block_y = 0; block_y < height_in_blocks_; ++block_y) {
    const coeff_t* src_coeffs = &comp.coeffs[block_y * src_row_size];
    for (int block_x = 0; block_x < width_in_blocks_; ++block_x) {
      for (int i = 0; i < kDCTBlockSize; ++i) {
        int j = (block_y * width_in_blocks_ + block_x) * kDCTBlockSize + i;
        blocks_mid[j] = src_coeffs[i] * quant[i];
        blocks_min[j] = src_coeffs[i] * quant[i] - (quant[i] / 2);
        blocks_max[j] = src_coeffs[i] * quant[i] + (quant[i] / 2);
        blocks_off[j] = 0;
      }
  ...
  // to show up.
  const int kLinearGradient[8] = {318,    -285,   81,    -32,
                                  17 * 0, -9 * 0, 5 * 0, -2 * 0};

  // Alpha(u) * sqrt(2) in 10-bit fixed-point precision.
  const int kAlphaSqrt2[8] = {1024, 1448, 1448, 1448, 1448, 1448, 1448, 1448};

  // Note: coefficients are 10 bits as well, so when the three are multiplied,
  // the number is 30-bit fixed point.

  // Compute adjustments to make the image continuous horizontally.
  for (int block_y = 0; block_y < height_in_blocks_; ++block_y) {
    for (int block_x = 0; block_x < width_in_blocks_ - 1; ++block_x) {
      int i = kDCTBlockSize * (block_y * width_in_blocks_ + block_x);
      int j = kDCTBlockSize * (block_y * width_in_blocks_ + block_x + 1);

      // Note that we only compensate discontenuities up to v = 3, not the
      // higher-frequency coefficients, because doing so extends artifacts,
      // which is worse than the discontinuity.
      for (int v = 0; v < 4; v++) {
        int64_t delta_v = 0;
        uint64_t hf_penalty = 0;

        for (int u = 0; u < 8; u++) {
          int alpha = kAlphaSqrt2[u];
          int sign = u & 1 ? -1 : 1;
          int gi = blocks_mid[i + v * 8 + u];
          int gj = blocks_mid[j + v * 8 + u];
          delta_v += alpha * (gj - sign * gi);
          hf_penalty += (u * u) * (gi * gi + gj * gj);
        }

        for (int u = 0; u < 8; u++) {
          // If there are high-frequency patterns along this edge, then
          // compensate those less, because otherwise we would extend artifacts
          // into adjacent blocks, which is worse than the discontinuity.
          if (hf_penalty > 400) delta_v /= 2;
          int sign = u & 1 ? 1 : -1;
          blocks_off[i + v * 8 + u] += delta_v * kLinearGradient[u];
          blocks_off[j + v * 8 + u] += delta_v * kLinearGradient[u] * sign;
        }
      }
    }
  }

  // Compute adjustments to make the image continuous vertically.
  for (int block_y = 0; block_y < height_in_blocks_ - 1; ++block_y) {
    for (int block_x = 0; block_x < width_in_blocks_; ++block_x) {
      int i = kDCTBlockSize * (block_y * width_in_blocks_ + block_x);
      int j = kDCTBlockSize * ((block_y + 1) * width_in_blocks_ + block_x);

      // Note that we only compensate discontenuities up to u = 3, not the
      // higher-frequency coefficients, because doing so extends artifacts,
      // which is worse than the discontinuity.
      for (int u = 0; u < 4; u++) {
        int64_t delta_u = 0;
        ...
        for (int v = 0; v < 8; v++) {
          int alpha = kAlphaSqrt2[v];
          int sign = v & 1 ? -1 : 1;
          int gi = blocks_mid[i + v * 8 + u];
          int gj = blocks_mid[j + v * 8 + u];
          delta_u += alpha * (gj - sign * gi);
          hf_penalty += (v * v) * (gi * gi + gj * gj);
        }

        for (int v = 0; v < 8; v++) {
          if (hf_penalty > 400) delta_u /= 2;
          int sign = v & 1 ? 1 : -1;
          blocks_off[i + v * 8 + u] += delta_u * kLinearGradient[v];
          blocks_off[j + v * 8 + u] += delta_u * kLinearGradient[v] * sign;
        }
      }
    }
  }

  // Sqrt(2) / 2 with 10 bits precision behind the point.
  const int kHalfSqrt2 = 724;

  // Add offsets computed this round, and clamp new coefficients to their
  // legal ranges.
  for (int i = 0; i < coeffs_size; i++) {
    // |blocks_off| contains the horizontal and vertical corrections. While in
    // isolation they remove discontinuities perfectly, doing so introduces new
    // discontinuities along the other edge. By weighing both with a factor
    // 1/sqrt(2), the correction works best on average. The offsets were
    // multiplied with two 10-bit fixed-point numbers, and |kHalfSqrt2| is 10
    // bits as well, so shift by 30 bits and one extra bit for the factor 1/2.
    blocks_mid[i] += (blocks_off[i] * kHalfSqrt2) >> 31;
    blocks_mid[i] = std::min(blocks_mid[i], blocks_max[i]);
    blocks_mid[i] = std::max(blocks_mid[i], blocks_min[i]);
  }

  for (int block_y = 0; block_y < height_in_blocks_; ++block_y) {
    for (int block_x = 0; block_x < width_in_blocks_; ++block_x) {
      int j = (block_y * width_in_blocks_ + block_x) * kDCTBlockSize;
      SetCoeffBlock(block_x, block_y, blocks_mid + j);
    }
  }
