...
// TODO Use SRGB->linear conversion and a lookup-table.
inline float GammaToLinear(float x) {
  return std::pow(x / 255.0, 2.2);
}

// TODO Use linear->SRGB conversion and a lookup-table.
...
void UpdateGuess(const std::vector<float>& target,
  ...
  for (int i = 0; i < guess->size(); ++i) {
    // TODO: Evaluate using a decaying constant here.
...
std::vector<std::vector<float> > RGBToYUV420(
  ...
  // TODO: Stop early if the error is small enough.
